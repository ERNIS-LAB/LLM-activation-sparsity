    ...
    for i, block in enumerate(model.blocks):
        for name in ['key', 'rec', 'val']:
            ...
            for cur_perc in perc_vals:
                block.att.sparsing_fns[name] = TopK(int(args.n_embd * cur_perc))
                ...
                if cur_loss / base_loss < loss_inc:
                    ...
                    break
            block.att.sparsing_fns[name] = TopK(int(args.n_embd * res_perc))
            base_loss = res_loss
            print(f"Block {i}, att, {name}, {res_perc:.4f}")

        res_loss, res_perc = base_loss, 1.0
        for cur_perc in perc_vals:
            block.att.sparsing_fns['out'] = TopK(int(args.n_embd * cur_perc))
            cur_loss = trainer.validate(model, dataloaders=data_loader, verbose=False)[0]['val_loss']
            if cur_loss / base_loss < loss_inc:
                res_loss, res_perc = cur_loss, cur_perc
            else:
                break
        block.att.sparsing_fns['out'] = TopK(int(args.n_embd * res_perc))
        base_loss = res_loss
        print(f"Block {i}, att, out, {res_perc:.4f}")
        
        for name in ['rec', 'key']:
            res_loss, res_perc = base_loss, 1.0
            for cur_perc in perc_vals:
                block.ffn.sparsing_fns[name] = TopK(int(args.n_embd * cur_perc))
                cur_loss = trainer.validate(model, dataloaders=data_loader, verbose=False)[0]['val_loss']
                if cur_loss / base_loss < loss_inc:
                    res_loss, res_perc = cur_loss, cur_perc
                else:
                    break
            block.ffn.sparsing_fns[name] = TopK(int(args.n_embd * res_perc))
            base_loss = res_loss
            print(f"Block {i}, ffn, {name}, {res_perc:.4f}")

        res_loss, res_perc = base_loss, 1.0
        for cur_perc in perc_vals:
            block.ffn.sparsing_fns['val'] = TopK(int(args.dim_ffn * cur_perc))
            cur_loss = trainer.validate(model, dataloaders=data_loader, verbose=False)[0]['val_loss']
            if cur_loss / base_loss < loss_inc:
                res_loss, res_perc = cur_loss, cur_perc
            else:
                break
        block.ffn.sparsing_fns['val'] = TopK(int(args.dim_ffn * res_perc))
        base_loss = res_loss
        print(f"Block {i}, ffn, val, {res_perc:.4f}")
