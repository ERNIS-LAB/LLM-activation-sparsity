...
if __name__ == "__main__":
    ...
    parser.add_argument("--valid_data_file", default="", type=str)
    ...
    ### Experiments arguements
    parser.add_argument("--freeze_mode", default=None, type=str)
    parser.add_argument("--sparsity", default=None, type=json.loads)
    parser.add_argument("--activation_reg", default=None, type=json.loads)
    ###
    ...
    if args.data_type == "wds_img":
        ...
        if getattr(args, 'sparsity') is not None or getattr(args, 'activation_reg') is not None:
            args.proj_dir = f"{args.proj_dir}/{args.wandb}"
            
            if not os.path.exists(args.proj_dir):
                os.makedirs(args.proj_dir)
            args.run_name = ''
            if getattr(args, 'sparsity') is not None:
                for k, v in args.sparsity.items():
                    args.run_name += f"{k}_{v}-"
            if getattr(args, 'activation_reg') is not None:
                args.run_name += f"-RegFn_{args.activation_reg['act_reg']}-lmbd_{args.activation_reg['act_reg_lambda']}"
            if getattr(args, 'freeze_mode') is not None:
                args.run_name += f"-Frz_{args.freeze_mode}"
            if args.lr_init != 1e-5:
                args.run_name += f"-lrInit_{args.lr_init}"
            if args.lr_final != 1e-5:
                args.run_name += f"-lrFinal_{args.lr_final}"
            args.proj_dir = f"{args.proj_dir}/{args.run_name}"
        else:
...
"""
    ...
    if pl.__version__[0]=='2':
        ...
        logger=args.logger,callbacks=[train_callback(args)],max_epochs=args.max_epochs,check_val_every_n_epoch=args.check_val_every_n_epoch,num_sanity_val_steps=args.num_sanity_val_steps, deterministic=True,
    ...
    if getattr(args, 'freeze_mode') is not None:
        if args.freeze_mode not in ['excl_thresh', 'only_thresh']:
            raise ValueError(f"Unknown freeze mode: {args.freeze_mode}")
        for name, param in model.named_parameters():
            if args.freeze_mode == 'excl_thresh' and 'thresholds' not in name:
                param.requires_grad = False
            elif args.freeze_mode == 'only_thresh' and 'thresholds' in name:
                param.requires_grad = False

    from src.dataset import ValDataset
    
    valid_data = ValDataset(args)
    args.vocab_size = valid_data.vocab_size

    val_dataloader = DataLoader(valid_data, shuffle=False, pin_memory=True, batch_size=args.micro_bsz, num_workers=1, persistent_workers=False, drop_last=False)
