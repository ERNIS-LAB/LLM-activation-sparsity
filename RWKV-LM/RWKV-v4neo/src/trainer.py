...
class train_callback(pl.Callback):
    ...
    def on_train_batch_start(self, trainer, pl_module, batch, batch_idx):
        ...
        if trainer.global_step == 0:
            if trainer.is_global_zero:  # logging
                if getattr(self.args, 'activation_reg') is not None:
                    trainer.my_act_reg_sum = 0
                    trainer.my_class_loss_sum = 0
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        if trainer.is_global_zero:  # logging
            ...
            if getattr(self.args, 'activation_reg') is not None:
                trainer.my_act_reg = trainer.act_reg_all.float().mean().item()
                trainer.my_act_reg_sum += trainer.my_act_reg
                trainer.my_epoch_act_reg = trainer.my_act_reg_sum / trainer.my_loss_count
                self.log("act_reg", trainer.my_epoch_act_reg, prog_bar=True, on_step=True)

                trainer.my_class_loss = trainer.class_loss_all.float().mean().item()
                trainer.my_class_loss_sum += trainer.my_class_loss
                trainer.my_epoch_class_loss = trainer.my_class_loss_sum / trainer.my_loss_count
                self.log("class_loss", trainer.my_epoch_class_loss, prog_bar=True, on_step=True)
            ...
            if len(args.wandb) > 0:
                ...
                if getattr(self.args, 'activation_reg') is not None:
                    lll["act_reg"] = trainer.my_act_reg
                    lll["class_loss"] = trainer.my_class_loss
                if kt_s > 0:
                    lll["kt/s"] = kt_s
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        if trainer.is_global_zero:
            for k, v in trainer.validation_metrics.items():
                trainer.validation_metrics[k] = torch.mean(torch.tensor(v)).item()

            if len(getattr(self.args, 'wandb', [])) > 0:
                trainer.my_wandb.log(trainer.validation_metrics, step=int(trainer.global_step + self.args.epoch_begin * self.args.epoch_steps))
            trainer.validation_metrics = {}
