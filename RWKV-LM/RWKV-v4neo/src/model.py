...
class RWKV_TimeMix(MyModule):
    def __init__(self, args, layer_id):
        ...
        self.sparsing_fns = nn.ModuleDict({
                'key': nn.Identity(), 
                'val': nn.Identity(),
                'rec': nn.Identity(),
                'out': nn.Identity()
        ...
        if getattr(args, 'sparsity', None) is not None:
            if 'sparsing_fn' in args.sparsity.keys():
                for key in self.sparsing_fns.keys():
                    self.sparsing_fns[key] = get_sparsing_fn(args, embed_dim=args.n_embd)
    
            self.sparse_stats:Dict[str, Dict] = {}
    ...
    if 'a' not in os.environ["RWKV_MY_TESTING"]:
        ...
        def jit_func(self, x):
            xx = self.time_shift(x) # Mix x with the previous timestep to produce xk, xv, xr
            xk = self.sparsing_fns['key'](x * self.time_mix_k + xx * (1 - self.time_mix_k))
            xv = self.sparsing_fns['val'](x * self.time_mix_v + xx * (1 - self.time_mix_v))
            xr = self.sparsing_fns['rec'](x * self.time_mix_r + xx * (1 - self.time_mix_r))

            if not self.training and self.sparse_stats is not None:
                self.sparse_stats['tm_key'] = calc_sparsity(xk)
                self.sparse_stats['tm_val'] = calc_sparsity(xv)
                self.sparse_stats['tm_rec'] = calc_sparsity(xr)
        ...
        def forward(self, x):
            ...
            rwkv = self.sparsing_fns['out'](sr * RUN_CUDA(B, T, self.args.dim_att, self.time_decay, self.time_first, k, v))
            # rwkv = sr * self.sparsing_fns['out'](RUN_CUDA(B, T, self.args.dim_att, self.time_decay, self.time_first, k, v))

            if not self.training and self.sparse_stats is not None:
                self.sparse_stats['tm_out'] = calc_sparsity(rwkv)
...
class RWKV_ChannelMix(MyModule):
    def __init__(self, args, layer_id):
        ...
        if getattr(args, 'sparsity', None) is not None:
            if 'sparsing_fn' in args.sparsity.keys():
                for key in self.sparsing_fns.keys():
                    if key == 'val':
                        self.sparsing_fns[key] = get_sparsing_fn(args, embed_dim=args.dim_ffn)
                    else:
                        self.sparsing_fns[key] = get_sparsing_fn(args, embed_dim=args.n_embd)
    ...
    def forward(self, x):
        ...
        if not self.training and self.sparse_stats is not None:
            self.sparse_stats['cm_key'] = calc_sparsity(xk)
            self.sparse_stats['cm_rec'] = calc_sparsity(xr)
            
        k = self.key(xk)
        k = torch.relu(k) ** 2
        k = self.sparsing_fns['val'](k)

        if not self.training and self.sparse_stats is not None:
            self.sparse_stats['cm_val'] = calc_sparsity(k)
...
class Block(nn.Module):
    def __init__(self, args, layer_id):
        ...
        self.sparse_stats = {} if getattr(args, 'sparsity', None) is not None else None
    ...
    def forward(self, x, x_emb=None):
        ...
        if not self.training and self.sparse_stats is not None:
            self.sparse_stats['att'] = self.att.sparse_stats
            self.sparse_stats['ffn'] = self.ffn.sparse_stats
        return x
...
class RWKV(pl.LightningModule):
    def __init__(self, args):
        ...
        self.act_reg_flag = False
        if getattr(args, 'activation_reg', None) is not None:
            self.act_reg_flag = True
        if self.act_reg_flag:
            self.sparsity_weight = args.activation_reg['act_reg_lambda']
            self.regularization = get_regularization_loss(args.activation_reg['act_reg'])
    ...
    def training_step(self, batch, batch_idx):
        ...
        if args.my_qa_mask != 1:
            ...
            if self.act_reg_flag:
                ...
                class_loss = loss.data.clone()
                loss += act_reg * self.sparsity_weight
        ...
        if self.act_reg_flag:
            return {'loss': loss, 'act_reg': act_reg.data.clone(), 'class_loss': class_loss}
        else:
            return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.trainer.validation_metrics['val_loss'] = self.trainer.validation_metrics.get('val_loss', []) + [loss.item()]
        if self.sparse_stats is not None and self.args.sparsity.get('sparsing_fn', '') == "hardshrink":
            threshold_avg = calc_threshold_avg(self)
            self.log("threshold_avg", threshold_avg, on_step=False, on_epoch=True, sync_dist=True, )
            self.trainer.validation_metrics['threshold_avg'] = self.trainer.validation_metrics.get('threshold_avg', []) + [threshold_avg]


        if self.sparse_stats is not None:
            for block in self.blocks:
                self.sparse_stats[str(block.layer_id)] = block.sparse_stats

            flat_dict = {}
            for key1, block in self.sparse_stats.items():
                for key2, mixing in block.items():
                    for key3, value in mixing.items():
                        flat_dict[f"{key1}_{key2}_{key3}"] = value
                        
            count = 0
            weighted_sparsity = 0.0
            for k, v in flat_dict.items():
                if 'cm_val' in k or 'cm_key' in k:
                    count += 4
                    weighted_sparsity += v * 4
                else:
                    count += 1
                    weighted_sparsity += v
            weighted_sparsity /= count
            
            self.log("weighted_sparsity", weighted_sparsity, on_step=False, on_epoch=True, sync_dist=True, )
            self.trainer.validation_metrics['weighted_sparsity'] = self.trainer.validation_metrics.get('weighted_sparsity', []) + [weighted_sparsity]
            self.log("sparsity_stats", flat_dict, on_step=False, on_epoch=True, sync_dist=True, )
    ...
    def training_step_end(self, batch_parts):
        if pl.__version__[0]!='2':
            ...
            if self.trainer.is_global_zero:
                if self.act_reg_flag:
                    self.trainer.act_reg_all = all['act_reg']
                    self.trainer.class_loss_all = all['class_loss']
                    self.trainer.my_loss_all = all['loss']
                else:
