...
class ValDataset(Dataset):
    def __init__(self, args):
        ...
        if getattr(args, "valid_data_file", "") != "":
            self.data = MMapIndexedDataset(args.valid_data_file)
        else:
            self.data = MMapIndexedDataset(args.data_file.replace("train", "validation"))
        self.data_size = len(self.data._bin_buffer) // self.data._index._dtype_size
        rank_zero_info(f"Data has {self.data_size} tokens.")
    ...
    def __len__(self):
        return self.data_size // (self.args.ctx_len + 1)
    ...
    def __getitem__(self, idx):
        ...
        idx = idx * (ctx_len + 1)
        dix = data.get(idx=0, offset=idx, length=req_len).astype(int)
