...
@dataclass
class IterTopKConfig(MetaseqDataclass):
    loss_inc: float = field(
        default=1.001,
        metadata={"help": "increase in loss each TopK function"}
    )
    start_k: float = field(
        default=0.9,
        metadata={"help": "starting k value"}
    )
    step: str = field(
        default="linear",
        metadata={"help": "step function to use"}
    )
    k_num: int = field(
        default=8,
        metadata={"help": "number of k values to use"}
    )
...
class MetaseqConfig(MetaseqDataclass):
    ...
    bpe: Any = MISSING
    tokenizer: Any = None
    # Add your custom dataclass here
    iter_topk: IterTopKConfig = IterTopKConfig()
