...
def main(cfg: DictConfig, override_args=None):
    ...
    for subset in cfg.dataset.valid_subset.split(","):
        ...
        for i, sample in enumerate(progress):
            ...
            if getattr(saved_cfg.model, 'sparsity', None) is not None:
                sparse_stats = model.decoder.sparse_stats
    ...
    if sparse_stats is not None:
        logger.info("Finished evaluating sparse metrics")
        logger.info([(k, sum(v) / len(v)) for k, v in sparse_stats.items()])
