...
def get_iter_topk_parser(default_task=None):
    parser = get_parser("Iterative Top-K", default_task)
    add_dataset_args(parser, train=True)
    add_distributed_training_args(parser, default_world_size=1)
    group = parser.add_argument_group("Evaluation")
    gen_parser_from_dataclass(group, CommonEvalConfig())
    group = parser.add_argument_group("Iterative Top-K")
    gen_parser_from_dataclass(group, IterTopKConfig())
    return parser
