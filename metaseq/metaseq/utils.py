...
class HardShrink(torch.nn.Module):
    def __init__(self, lambd: float = 0.5, embed_dim: int = 768, dtype=torch.float32, learnable=False):
        super().__init__()
        self.thresholds = torch.nn.Parameter(torch.full((embed_dim, ), lambd, dtype=dtype))
...
def get_sparsing_fn(args, embed_dim: int = 768, dtype=torch.float32):
    if args.sparsity['sparsing_fn'] == "topk":
        ...
        return HardShrink(lambd=args.sparsity.get('hardshrink_lambda', 0.0), embed_dim=embed_dim, dtype=dtype)
