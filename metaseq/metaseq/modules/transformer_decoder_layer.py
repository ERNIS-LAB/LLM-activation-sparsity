...
class ModelParallelTransformerDecoderLayer(nn.Module):
        ...
        if getattr(args, 'sparsity', None) is not None:
            if 'sparsing_fn' in args.sparsity.keys():
                for key in self.sparsing_fns.keys():
                    if key == 'down_proj':
                        self.sparsing_fns[key] = utils.get_sparsing_fn(args, embed_dim=args.decoder_ffn_embed_dim, dtype=dtype)
                    else:
                        self.sparsing_fns[key] = utils.get_sparsing_fn(args, embed_dim=self.embed_dim, dtype=dtype)
    ...
    def forward(
        ...
        qkv = self.sparsing_fns['qkv'](x)
        if not self.training and self.sparse_stats is not None:
            self.sparse_stats['qkv'] = utils.calc_sparsity(qkv)
        ...
        x = FeedForward(
            ...
            sparsing_fns=self.sparsing_fns,
            sparse_stats=self.sparse_stats if not self.training else None,
        )
        x = residual + x
        return x
